...
' //
' // File:      DeployWiz_LanguageUIOnline.vbs
' // 
...
function oAddLanguage( LanugageToAdd )
	...
	For each oLang in oXMLLanguageList.selectNodes("//LOCALEDATA/LOCALE[IFLAGS='0' and SNAME]")

		If ucase(oUtility.SelectSingleNodeString(oLang,"SENGDISPLAYNAME")) = ucase(LanugageToAdd) then
			Set oOption2 = document.createElement("OPTION")
			oOption2.Text = oUtility.SelectSingleNodeString(oLang,"SENGDISPLAYNAME")
			If not oLang.SelectSingleNode("SSPECIFICCULTURE") is nothing then
				oOption2.Value = lcase(oUtility.SelectSingleNodeString(oLang,"SSPECIFICCULTURE"))
...
Function oLocale_Initialization
	...
	oLogging.CreateEntry "###### Locale_Initialization_Online ###### " , LogTypeInfo
	...
	' Set Windows 10 Name
	oLogging.CreateEntry "Windows edition" & oEnvironment.Item("LatestOnlineVersionName") , LogTypeInfo
	W10Name.InnerHTML = oEnvironment.Item("LatestOnlineVersionName") & " Language:"

	' Parse Langugaes
	oLogging.CreateEntry  oEnvironment.Item("RawOnlineLanguageList") , LogTypeInfo
	olines=split(oEnvironment.Item("RawOnlineLanguageList"),",")
	AllreadyAddedLanguages = ""
	
	for i=0 to ubound(olines)
		oAddLanguage olines(i)
		AllreadyAddedLanguages = AllreadyAddedLanguages & vbTab & olines(i)
	next
    ...
    oLogging.CreateEntry "UILanguage: " & property("UILanguage") , LogTypeinfo
	
	' Populate the Locale
	For each oItem in oXMLLanguageList.selectNodes("//LOCALEDATA/LOCALE[IFLAGS='0']")

		Set oOption2 = document.createElement("OPTION")
		oOption2.Text = oUtility.SelectSingleNodeString(oItem,"SENGDISPLAYNAME")

		oOption2.Value = lcase(oUtility.SelectSingleNodeString(oItem,"./SSPECIFICCULTURE") & ";" & oUtility.SelectSingleNodeString(oItem,"IINPUTLANGUAGEHANDLE"))
...
Function SetEditionToPRO
	ISWinEnt.Checked = False
	oEnvironment.Item("EditionToInstall") = "Professional"
	oEnvironment.Item("ProductKey") = "W269N-WFGWX-YVC9B-4J6C9-T83GX"
End Function

Function SetEditionToEnt
	ISWinPro.Checked = False
	oEnvironment.Item("EditionToInstall") = "Enterprise"
	oEnvironment.Item("ProductKey") = "NPPR9-FWDCX-D2C8J-H872K-2YT43"
End Function
...
Function SetNewLocaleEx (thisLocale)
	...
	Dim sItem
	Dim sNew
	oLogging.CreateEntry "SetNewLocaleEx " & thisLocale & " - " & Property("KeyboardLocale"), LogTypeInfo
	...
	oLogging.CreateEntry "KeyboardLocale: " & Property("KeyboardLocale") & " - " & KeyboardLocale_Edit.Value & " - " & sNew, LogTypeinfo
...
Function oLocale_Validation

	Dim iSplit

	oLocale_Validation = TRUE
	...
	If not oLocale_Validation then
		Exit Function
	End if
...
Function oTimeZone_Initialization
